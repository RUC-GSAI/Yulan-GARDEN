...
from utils.dumper import dump_jsonl
...
def _calculate_work_count(works, input_ext: str) -> int:
    if input_ext in TXT_SUFFIX:
        ...
        raise Exception("No valid input extension is given in _calculate_work_count..\n")
    return work_count
    ...
    if input_ext in TXT_SUFFIX:
        for work in tqdm(works, desc="Generating .tmp files"):
            ...
            if cnt >= tmp_file_line:
                dump_jsonl(path=os.path.join(tmp_path, f'{split}.jsonl'), data=res, source_tag=source_tag)
