...
import transformers
from transformers import AutoModelForCausalLM, GPT2Tokenizer, AutoTokenizer, HfArgumentParser, get_scheduler, set_seed

os.environ["WANDB_API_KEY"] = "c012d967fb7c828eec6382081e44960e7e017b47"
...
def create_dataloaders(args):
    ...
    train_data = load_dataset("json", split="train", data_files=args.dataset_name_train)
    # train_data = load_dataset(args.dataset_name_train, split="train", **ds_kwargs)
    # print(type(train_data), train_data.num_rows, train_data.column_names, train_data.num_rows)
    # if type(train_data) is DatasetDict:
    train_data = train_data.shuffle(seed=args.seed)
    # else:
        # train_data = train_data.shuffle(buffer_size=args.shuffle_buffer, seed=args.seed)
    valid_data = load_dataset("json", split="train", data_files=args.dataset_name_valid)
...
logger.info("begin to prepare train/eval dataloader..")
train_dataloader, eval_dataloader = create_dataloaders(args)
logger.info("prepare train/eval dataloader finished..")
...
logger.info("prepare optimizer finished..")
