...
def process_work(conf: Settings, logger: Logger):
    ...
    if settings['if_filter'] or settings['if_clean'] or settings['if_debug']:
        ...
        if settings['if_dedup']: dedupicator_module = Deduplicator(settings)
        else: dedupicator_module = None
        ...
        if settings['if_debug']:
            ...
            logger.log_text(f"generating debug report {debugger_module.debug_report_path} finish..")
